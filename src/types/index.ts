...
interface BpmnElementBase {
  ...
  size: Size;
  /**
   * True when `size` came from the type default rather than the caller.
   * Auto-layout treats a caller-supplied size as a floor it must not shrink
   * below; a defaulted one carries no such intent and may be replaced.
   */
  sizeManaged?: boolean;
  properties: Record<string, unknown>;
}
...
/** One element in a batched build, addressed by a caller-chosen handle. */
export interface BuildNodeRequest extends ElementDefinition {
  /**
   * Caller-side handle used by flows in the same request. It never reaches the
   * BPMN document; the server assigns the real id and reports the mapping.
   */
  ref: string;
}

/** One connection in a batched build. */
export interface BuildFlowRequest extends BpmnConnectOptions {
  /** A `ref` from the same request, or the id of an element already present. */
  source: string;
  /** A `ref` from the same request, or the id of an element already present. */
  target: string;
  label?: string;
...
export interface ProcessContext {
  ...
  filename?: string;
  /**
   * True when `filename` was generated by the server rather than chosen by the
   * caller. A generated name is a placeholder, so save_as replaces it instead
   * of leaving an orphaned duplicate behind.
   */
  filenameManaged?: boolean;
