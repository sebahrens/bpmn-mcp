...
After completion, report the active BPMN filename from the latest structured result. If the user asked for SVG, also identify the returned SVG resource/URI and its export status without presenting it as a saved `.svg` file.
